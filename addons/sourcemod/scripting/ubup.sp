			...
			for (new client_id = 1; client_id < MAXPLAYERS + 1; client_id++)
			{
				if (IsValidClient(client_id))
				{
					...
					&& client_id != assist)
					{
						...
						PrintToChat(client_id, "團隊殺敵! +%d$", iCash_forteam)
					}
					else if (client_id != client && GetClientTeam(client_id) == team_c)
					{
						...
						SetEntProp(client_id, Prop_Send, "m_nCurrency", iCashtmpb);
						client_iCash[client_id] = iCashtmpb
						PrintToChat(client_id, "團隊殺敵 +%d$ 獲得 ", iCashtmpc)
					}
				}
			}
		}
				...
				&& given_upgrd_list[w_id][arg2_][arg3_])
				{
					...
					if (inum != 9999 && upgrades_ratio[upgrade_choice])
					{
						...
						if (iCash < t_up_cost)
						{
							...
							Format(buffer, sizeof(buffer), "%T", "窮!!", mclient);
						...
						else
						{
							...
							else
							{
								...
								if (ctr_m != clientLevels[mclient])
								{
									...
									{
										Format(strsn, sizeof(strsn), "[★%d]", ctr_m + 1)
									}
		...
		for (new i = 0; i < upgrades_tweaks_nb_att[spTweak]; i++)
		{
			...
			if (inum != 9999)
				...
				else
				{
					...
					{
						PrintToChat(mclient, "你沒有額外欄位可以使用.");
						got_req = 0
					}
				}
			...
			if (param2 >= 0)
			{
				...
				if (u != 9999)
				{
					if (upgrades_costs[u] < -0.0001)
					{
						...
						{
							new String:buffer[64]
							Format(buffer, sizeof(buffer), "%T", "窮!!", client);
							PrintToChat(client, buffer);
						}
					}
					else
					{
						PrintToChat(client,"母湯.")
					}
				}
			}
		}
